...
class UsersConfigDAO:
    ...
    def config_user_to_database(self, params: tuple) -> None:
        query: str = '''INSERT IGNORE INTO users_config(user_id, only_new, max_size, language, price_filter, name_filter) VALUES(%s, %s, %s, %s, %s, %s)'''
    ...
    def change_only_new_config(self, only_new: str, user_id: int) -> None:
        query: str = '''UPDATE users_config SET only_new = %s WHERE user_id = %s'''
        with self.database as cursor:
            cursor.execute(query, (only_new, user_id))

    def change_name_filter_config(self, name_filter: str, user_id: int) -> None:
        query: str = '''UPDATE users_config SET name_filter = %s WHERE user_id = %s'''
        with self.database as cursor:
    ...
    def change_price_filter_config(self, price_filter: str, user_id: int) -> None:
        query: str = '''UPDATE users_config SET price_filter = %s WHERE user_id = %s'''
        with self.database as cursor:
    ...
    def change_lang_config(self, language: str, user_id: int) -> None:
        query: str = '''UPDATE users_config SET language = %s WHERE user_id = %s'''
        with self.database as cursor:
    ...
    def get_all_configs(self, user_id: int) -> UserConfig:
        query: str = '''SELECT only_new, max_size, language, price_filter, name_filter FROM users_config WHERE user_id = %s'''
        with self.database as cursor:
    ...
    def change_max_size_config(self, user_id: int, max_size: int) -> None:
        query: str = '''UPDATE users_config SET max_size = %s WHERE user_id = %s'''
        with self.database as cursor:
